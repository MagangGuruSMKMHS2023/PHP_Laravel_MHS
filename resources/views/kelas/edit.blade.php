...
<form class="w-full max-w-sm" method="post" action="/kelas/{{$kelas->id_kelas}}" enctype="multipart/form-data">
@method("put")  
@csrf
  ...
  <div class="md:flex md:items-center mb-6">
    ...
    <div class="md:w-2/3">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" type="text"
       name="namakelas" value="{{ $kelas->namakelas }}" required>
    </div>
  </div>
  <div class="md:flex md:items-center mb-6">
    <div class="md:w-1/3">
    ...
    <select class="bg-gray-200 border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" type="text"
       ...
       ?>
        <option value="<?= $row['id_guru'] ?>" <?php if($kelas->walikelas ==  $row['id_guru'] ) echo 'selected = "selected"';?>><b><?= $row['namaguru'] ?></b></option>
  ...
  <div class="md:flex md:items-center mb-6">
    ...
    <div class="md:w-2/3">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" type="text"
       name="kursi" value="{{ $kelas->kursi }}" required>
    </div>
  </div>
  <div class="md:flex md:items-center mb-6">
    <div class="md:w-1/3">
    ...
    <div class="md:w-2/3">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500" id="inline-full-name" type="text"
       name="meja" value="{{ $kelas->meja }}" required>
    </div>
  </div>
  <div class="md:flex md:items-center mb-6">
    <div class="md:w-1/3">
    ...
    <div class="md:w-2/3">
        ...
        <img src="{{ asset('upload_gambar/'. $kelas->gambar_kelas)}}" alt="" width="100%" height="100%">
        @else
